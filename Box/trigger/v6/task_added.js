...
var Event = {
  "file": ["TASK_ASSIGNMENT.CREATED"],
  "folder": ["TASK_ASSIGNMENT.CREATED"]
};
...
function _output(input, data) {
  try {
    ...
    data = Object.assign({ "event_type": "Task Added" }, data)
      ...
      switch (input.event) {
        case "folder":
          ...
          for (var i = 0; i < body.item_collection.entries.length; i++) {
            if (body.item_collection.entries[i].type == "ile")
