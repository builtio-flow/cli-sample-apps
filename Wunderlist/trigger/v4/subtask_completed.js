...
module.exports = {
	...
	mock_data: {
		...
		"completed_user_name": "Michel Addams",
		"completed_user_email": "micheladdams@example.com",
	...
	execute: function(input, payload, output) {
		...
		if (!payload || (payload.operation && payload.operation !== "update") || (payload.subject && payload.subject.type !== "subtask") || (payload.data && !payload.data.completed) || (input.wunderlist_tasks && input.wunderlist_tasks != payload.after.task_id)) {
			return output(null, []);
		}
		Promise.all([
			...
			outputObj.event_type = "Subtask Completed";
			...
			setValues(payload.data, outputObj, "completed_by_id", "completed_user_id");
			promiseData[1] && promiseData[1].length && setValues(promiseData[1].filter(user => user.id == payload.data.completed_by_id)[0], outputObj, "name", "completed_user_name");
			promiseData[1] && promiseData[1].length && setValues(promiseData[1].filter(user => user.id == payload.data.completed_by_id)[0], outputObj, "email", "completed_user_email");
			setValues(payload.data, outputObj, "completed_at");
