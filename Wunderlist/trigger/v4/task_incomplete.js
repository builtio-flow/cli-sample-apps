...
module.exports = {
	...
	execute: function(input, payload, output) {
		...
		if (!payload || (payload.operation && payload.operation !== "update") || (payload.subject && payload.subject.type !== "task") || payload.data.completed || !payload.before.completed) {
			return output(null, []);
		}
		Promise.all([
			...
			outputObj.event_type = "Task Incomplete";
