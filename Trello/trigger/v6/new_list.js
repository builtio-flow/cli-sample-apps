...
module.exports = {
  ...
  mock_data: {
    ...
    "action": {
      ...
      "list_id": "5909666f9b9dcc39e1b81e0a",
  ...
  execute: function (input, payload, output) {
    ...
    getDetails(input, payload, function (err, res) {
      if (!err && res && res.action && res.action.type == "createList" && res.action.display && res.action.display.translationKey == "action_added_list_to_board") {
        ...
        res = Object.assign({ "event_type": "New List" }, res)
        output(null, res);
      } else {
        output(null, []);
      }
