...
module.exports = {
  ...
  mock_data: {
    "event_type": "Delete Card",
    "model": {
      "id": "58fef3816daf33593cc95661",
      "name": "New Board",
      "desc": "This is description for card",
      "closed": false,
    ...
    "action": {
      ...
      "list_name": "Doing",
      "list_id": "5bd008df5beaaf6b84d9e130",
  ...
  execute: function (input, payload, output) {
    ...
    getDetails(input, payload, function (err, res) {
      if (input.list_id && input.list_id != "" && res.action.list_id && res.action.list_id != input.list_id) {
        output(null, []);
      } else if (!err && res && res.action && res.action.type == "deleteCard") {
        ...
        res = Object.assign({ "event_type": "Delete Card" }, res)
        output(null, res);
      } else {
        output(null, []);
      }
