...
module.exports = {
  ...
  mock_data: {
    ...
    "model": {
      ...
      "name": "New Board",
      "desc": "This is description for new card",
      "closed": false,
    ...
    "action": {
      ...
      "list_id": "5bd008df5beaaf6b84de130",
  ...
  execute: function (input, payload, output) {
    ...
    getDetails(input, payload, function (err, res) {
      if (input.list_id && input.list_id != "" && res.card && res.card.idList && res.card.idList != input.list_id) {
        output(null, []);
      } else if (!err && res && res.action && res.action.type == "createCard") {
        ...
        res = Object.assign({ "event_type": "New Card" }, res)
        output(null, res);
      } else {
        output(null, []);
      }
