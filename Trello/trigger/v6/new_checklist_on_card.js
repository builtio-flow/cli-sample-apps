...
module.exports = {
  ...
  name: "new_checklist_on_card",

  label: "New Checklist On Card",

  version: "v6",

  input: {
    type: 'object',
    title: 'New Checklist On Card',
    description: "Short description",
    properties: {
      event: {
        type: 'string',
        enum: ['new_checklist_on_card']
      },
      polling: {
        type: 'boolean',
        default: false,
  ...
  mock_data: {
    "event_type": "New Checklist On Card",
    "model": {
    ...
    "action": {
      ...
      "display": {
        "translationKey": "action_add_checklist_to_card",
        "entities": {
          "checklist": {
            "type": "checklist",
  ...
  execute: function (input, payload, output) {
    ...
    getDetails(input, payload, function (err, res) {
      if (input.list_id && input.list_id != "" && res.card && res.card.idList && res.card.idList != input.list_id) {
        output(null, []);
      } else if (!err && res && res.action && res.action.type == "addChecklistToCard" && res.action.display && res.action.display.translationKey == "action_add_checklist_to_card") {
