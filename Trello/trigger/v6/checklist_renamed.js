...
module.exports = {
  ...
  name: "checklist_renamed",

  label: "Checklist Renamed",
  ...
  mock_data: {
    ...
    "action": {
      ...
      "display": {
        "translationKey": "action_renamed_checklist",
        "entities": {
          "checkitem": {
            "type": "checkItem",
            "id": "5b5ed8cd98aff935765e6f99",
            "text": "New Checklist"
          },
          "name": {
            "type": "text",
      ...
      "checklist_name": "List of items to be added",
  ...
  execute: function (input, payload, output) {
    ...
    //"updateCheckItem" action_renamed_checkitem
    // your code goes here
    getDetails(input, payload, function (err, res) {
      if (input.list_id && input.list_id != "" && res.card && res.card.idList && res.card.idList != input.list_id) {
        output(null, []);
      } else if (!err && res && res.action && res.action.type == "updateChecklist" && res.action.display && res.action.display.translationKey == "action_renamed_checklist") {
...
function getDetails(input, data, cb) {
  ...
  _flatten(data, function (res) {
    opt.url = "https://api.trello.com/1/checklists/" + (res.action.checklist_id || res.action.id) + "/cards"
