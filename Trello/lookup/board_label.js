	...
	module.exports = {
		...
		'execute': function (input, options, output) {
			var qs = {}
			var baseURL = "https://api.trello.com/1/boards/"+input.board_id+"/labels?filter=all&fields=name,color"
			var flag = false
		...
		if (Array.isArray(body)) {
			if (input.search) {// searchByValue API not provided for list and members, so getting all result and returning filtered data based on searched value 
				...
				lodash.map(body, function (item) {
					if (regexExp.test(item.name || item.color)) {
						result.push({
							"id": String(item.id),
							"value": item.name? item.color+"("+ item.name+")" : item.color
						});
					}
				})
