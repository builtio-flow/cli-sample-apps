...
module.exports = {
  ...
  mock_input: {
    search: "from:kuldeep.kushwaha@built.io",
    label: "INBOX"
    //  label: "Label_2811454157892320634"
  },
  ...
  execute: function (input, options, output) {
    ...
    async function call() {
      try {
        ...
        if (mail && mail.data && mail.data.resultSizeEstimate && mail.data.resultSizeEstimate > 0) {
          mail.data.messages.forEach(element => {
            label_id_arr.push(element.id)
            if (options.meta.label_id.indexOf(element.id) == -1) {
              mail_data_arr.push(comm.users_messages_get(gmail, element.id))
            }
          })
          Promise.all(mail_data_arr).then(res => {
            res.length && res.map(curr => {

              final_data.push(comm.make_object(user_email, input.label, user_labels, curr.data, "New Labeled Email"))
            })
  ...
  validate: function (input, options, output) {
    ...
    async function call() {
      try {
        ...
        //---------------------------------------------------------------
        if (mail && mail.data && mail.data.resultSizeEstimate) {
          if (mail && mail.data && mail.data.messages && mail.data.messages.length) {

            var label_id_arr = mail.data.messages.map(curr => curr.id)


            var mail_data = await comm.users_messages_get(gmail, mail.data.messages[0].id)
            ...
            if (mail_data.data) {
              return output(null, mail_data.data)
            } else {
